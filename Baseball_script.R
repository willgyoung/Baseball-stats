...
model <- train(tagged_pitch_type ~ count + outs + pa_of_inning + pitcher + batter_side,
               data = train.data,
               method = "multinom",
               trControl = myCtrl)
...
train.data2 <- all_pitches[training.samples,]
test.data2 <- all_pitches[-training.samples,]  
...
names(all_pitches)[15] <- "Pitch_count"
